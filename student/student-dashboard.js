...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // --- MODIFICATION START ---
  // Load all buses by default when the page loads
  displayAllBusRoutes();
  // --- MODIFICATION END ---
...
function searchBuses(searchType) {
  ...
  // Use the local window.busData instead of fetching from an API
  let filteredBuses = window.busData;
  ...
  if (query) {
    filteredBuses = filteredBuses.filter(bus =>
      ...
      bus.currentStop.toLowerCase().includes(query) ||
      bus.nextStop.toLowerCase().includes(query)
    );
  }
  if (routeFilter) {
    filteredBuses = filteredBuses.filter(bus => bus.route === routeFilter);
  }
  if (statusFilter) {
  ...
  // Apply sorting
  if (sortBy === 'eta') {
    filteredBuses.sort((a, b) => a.etaMinutes - b.etaMinutes);
  } else if (sortBy === 'route') {
    filteredBuses.sort((a, b) => a.route.localeCompare(b.route));
  } else if (sortBy === 'occupancy') {
    filteredBuses.sort((a, b) => a.occupancy - b.occupancy);
  }
...
function displaySearchResults(buses) {
  ...
  if (!buses || buses.length === 0) {
    resultsContainer.innerHTML = `<div class="no-results" style="text-align:center; padding: 20px;">
                                      <p>No buses found matching your criteria.</p>
                                  </div>`;
    return;
  }
  // The busData is already in the correct format, no need to normalize
...
function displayAllBusRoutes() {
    // This function now renders all buses from the local data source by default
    const allBuses = window.busData || [];
    displaySearchResults(allBuses);
}
...
function createBusCard(bus) {
  ...
  // --- Start of new code for the tracker ---
  const routeInfo = window.busRoutesData.find(route => route.number === bus.route);
  const stops = routeInfo ? routeInfo.stops : [];
  const stopsHTML = stops.map(stop => `<div class="route-stop" data-stop-name="${stop.pickup}"></div>`).join('');

  let busPositionPercentage = 0;
  if (stops.length > 1) {
      const currentStopIndex = stops.findIndex(stop => stop.pickup === bus.currentStop);
      if (currentStopIndex !== -1) {
          busPositionPercentage = (currentStopIndex / (stops.length - 1)) * 100;
      }
  }
  ...
  const progressTrackerHTML = `
      <div class="route-progress-container">
        ...
        <div class="bus-icon" style="left: ${busPositionPercentage}%;">🚌</div>
      </div>
  `;
  // --- End of new code for the tracker ---
  ...
  return `
    <div class="bus-card" tabindex="0" aria-label="Bus ${bus.number}, status ${bus.status}">
      ...
      ${progressTrackerHTML} {/* <-- Tracker is added here */}
